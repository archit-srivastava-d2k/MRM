...
const SelectTrainingData = ({ filterConditions, setFilterConditions, totalRecords, filteredRecords, setFilteredRecords }) => {
  const [useAllRecords, setUseAllRecords] = React.useState(false);

  const addFilterCondition = () => {
    setFilterConditions([...filterConditions, { field: '', operator: 'Contains', value: [] }]);
    setUseAllRecords(false);
    setFilteredRecords(filteredRecords); // Maintain current filtered count
  };

  const removeFilterCondition = (index) => {
    const newConditions = filterConditions.filter((_, i) => i !== index);
    setFilterConditions(newConditions);
    // Update filteredRecords based on remaining conditions (simulated logic)
    setFilteredRecords(newConditions.length === 0 ? totalRecords : filteredRecords);
  ...
  const handleSelectAllRecords = () => {
    setUseAllRecords(true);
    setFilterConditions([]); // Clear filters
    setFilteredRecords(totalRecords); // Set to total records
  };

  const handleSelectFilteredSet = () => {
    setUseAllRecords(false);
    if (filterConditions.length === 0) {
      setFilterConditions([{ field: '', operator: 'Contains', value: [] }]);
    }
    setFilteredRecords(filterConditions.length === 0 ? totalRecords : filteredRecords);
  };
    ...
    <div className="max-w-4xl">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && handleSelectAllRecords()}
          >
          ...
          {/* Filtered Set */}
          <div
            className={`p-6 border-2 rounded-lg relative cursor-pointer transition-all duration-200 ${
              !useAllRecords
                ? 'border-blue-500 bg-blue-50 shadow-md'
                : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'
            }`}
            onClick={handleSelectFilteredSet}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && handleSelectFilteredSet()}
          >
            <div className="flex items-start space-x-4">
              <div className="flex-shrink-0 mt-1">
                <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
        ...
        {/* Condition Requirements Section (Hidden when All Records is selected) */}
        {!useAllRecords && (
            ...
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <select
                  ...
                  defaultValue="All Conditions Are Met"
                >
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setFilterConditions(newConditions);
                            setFilteredRecords(Math.max(1, filteredRecords - 10)); // Simulated reduction
                          }}
                        >
                    ...
                    <div className="col-span-3">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setFilterConditions(newConditions);
                          setFilteredRecords(Math.max(1, filteredRecords - 5)); // Simulated reduction
                        }}
                      >
                            ...
                            <span key={i} className="inline-flex items-center px-2 py-1 bg-gray-100 text-sm rounded-md">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setFilterConditions(newConditions);
                                  setFilteredRecords(Math.min(totalRecords, filteredRecords + 5)); // Simulated increase
                                }}
      ...
      <div className="flex items-center justify-between mt-6 px-6 py-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <button
          ...
          onClick={() => setFilteredRecords(useAllRecords ? totalRecords : Math.max(1, filteredRecords - 10))} // Simulated refresh
        >
          Refresh
        </button>
      </div>
